...
from sqlalchemy import (Column, Integer, String, Double, ForeignKey, 
                        TIMESTAMP, JSON, Float, UniqueConstraint, create_engine)
...
class Campus(Base): 
    ...
    name = Column(String(255), nullable=False, index=True) # name VARCHAR(255) NOT NULL, UNIQUE
    ...
    __table_args__ = (
        UniqueConstraint('name', 'city', 'state', name='_name_city_state_uc'),
    )
